...
export const useSchedule = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      const raceCategoryFilter = encodeURIComponent(`category[equals]wirz3e8ue_y`);
      const eventCategoryFilter = encodeURIComponent(`category[equals]kf-0ocxoj15g`);
      const urlWithRaceFilter = `${apiUrl}?filters=${raceCategoryFilter}&limit=6&orders=-publishedAt`;
    const urlWithEventFilter = `${apiUrl}?filters=${eventCategoryFilter}&limit=6&orders=-publishedAt`;
